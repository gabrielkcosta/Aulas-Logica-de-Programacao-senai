...
const modo = keyboard.questionInt("\nEscolha o modo: ");
...
switch (modo) {
    case 1: {
        ...
        for (let i = 1; i <= MAX_TENTATIVAS; i++) {
            let palpite = keyboard.questionInt(`Tentativa ${i}: `);

            if (palpite === secreto) {
                console.log(`Parabens! Voce acertou em ${i} tentativa(s)!`);
                venceu = true;
                break;
    ...
    case 2: {
        ...
        for (let i = 1; i <= MAX_TENTATIVAS; i++) {
            ...
            if (resposta === 1) {
                console.log(`Vitoria! Acertei em ${i} tentativas.`);
                acertou = true;
                break;
            } else if (resposta === 2) {
                max = palpite - 1;
        ...
        if (!acertou) {
            console.log("O computador nao conseguiu adivinhar. O jogo terminou.");
        }
        break;
    }
